# coding=utf-8


import unittest
import requests
from ifautotest.ifmain.common import config, common_lib, dgq_route_params


class IndexTopicList(unittest.TestCase):
    """推荐话题列表"""

    def setUp(self):
        self.result = None
        self.common_headers = common_lib.get_common_header(type="dgq")
        self.url = "http://{0}/{1}".format(config.URL.DGQ_URL, dgq_route_params.JllTopicIndexTopicList.route)
        self.params = dgq_route_params.JllTopicIndexTopicList.params

    def tearDown(self):
        print("IndexTopicList: ", self.result)

    # @unittest.skip("11")
    def test_index_topic_list(self):
        """推荐话题列表"""
        _headers = common_lib.get_secret_header(self.url, self.common_headers, self.params)
        req = requests.post(self.url, headers=_headers)
        self.result = req.json()

        # 断言相关参数，是否正确
        self.assertEqual(str(self.result["errorCode"]), "0", msg="errorCode")


if __name__ == "__main__":
    unittest.main()
