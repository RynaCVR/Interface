...
class JllUsersGetColleague(unittest.TestCase):
    """我的同事列表"""

    def setUp(self):
        self.result = None
        self.common_headers = common_lib.get_common_header(type="dgq")
        self.url = "http://{0}/{1}".format(config.URL.DGQ_URL, dgq_route_params.JllUsersGetColleague.route)
        self.params = dgq_route_params.JllUsersGetColleague.params

    def tearDown(self):
        print("JllUsersGetColleague:\n ", self.result)
