...
class JllUsersChangeMobile(unittest.TestCase):
    """更换手机号"""

    def setUp(self):
        self.result = None
        self.common_headers = common_lib.get_common_header(type="dgq")
        self.url = "http://{0}/{1}".format(config.URL.DGQ_URL, dgq_route_params.JllUsersChangeMobile.route)
        self.params = dgq_route_params.JllUsersChangeMobile.params

    def tearDown(self):
        print("JllUsersChangeMobile:\n ", self.result)
